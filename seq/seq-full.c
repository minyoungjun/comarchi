...
int gen_dstE()
{
    return ((((icode) == (I_RRMOVL)) & (cond)) ? (rb) : ((icode) == 
        ...
        (I_POPL) || (icode) == (I_CALL) || (icode) == (I_RET)) ? (REG_ESP)
       : ((icode) == (I_LEAVE)) ? (REG_ESP) : (REG_NONE));
}
...
int gen_aluA()
{
    return (((icode) == (I_RRMOVL) || (icode) == (I_ALU)) ? (vala) : (
      ...
      (valc) : ((icode) == (I_CALL) || (icode) == (I_PUSHL)) ? -4 : ((icode)
         == (I_RET) || (icode) == (I_POPL)) ? 4 : ((icode) == (I_LEAVE)) ? 4
       : 0);
}
...
int gen_aluB()
{
    return (((icode) == (I_RMMOVL) || (icode) == (I_MRMOVL) || (icode) == 
        ...
        (icode) == (I_ISUBL)) ? (valb) : ((icode) == (I_LEAVE)) ? (valb) : 
      ((icode) == (I_RRMOVL) || (icode) == (I_IRMOVL)) ? 0 : 0);
}

int gen_alufun()
{
    return (((icode) == (I_ALU)) ? (ifun) : ((icode) == (I_ISUBL)) ? 
      (A_SUB) : (A_ADD));
}
...
int gen_mem_addr()
{
    return (((icode) == (I_RMMOVL) || (icode) == (I_PUSHL) || (icode) == 
        (I_CALL) || (icode) == (I_MRMOVL)) ? (vale) : ((icode) == (I_POPL)
         || (icode) == (I_RET)) ? (vala) : ((icode) == (I_LEAVE)) ? (vala)
       : 0);
}
